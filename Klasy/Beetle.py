...
class Beetle:
    ...
    def update(self, delta_time):
        ...
        movement = base_move * random_factor * boost_factor * 0.5
        self.x += movement
...
class BeetleRaceGame:
    def __init__(self, player):
        ...
        self.exit_button_rect = pygame.Rect(SCREEN_WIDTH - 100, 10, 90, 30)
        # Wyścig
    ...
    def init_beetles(self):
        ...
        beetle_names = ["Luigi Bugatti", "Zygzak McŻuk", "Hudson Żuk Hornet", "MaŻuk Marucha", "Żulari"]
        beetle_colors = [
            (255, 50, 50), (255, 255, 50), (50, 255, 50), 
            (50, 150, 255), (200, 50, 255)
        ]
        
        self.beetles = []
        lane_height = self.track_rect.height / len(beetle_names)
        
        for i in range(len(beetle_names)):
            lane_y = self.track_rect.y + (i + 0.5) * lane_height
            
            # Losowe wartości dla każdego żuka
            random_speed = random.uniform(0.7, 1.5)  # Prędkość między 0.7 a 1.5
            random_odds = round(random.uniform(1.8, 4.5), 1)  # Kursy między 1.8 a 4.5
            
            beetle = Beetle(
                x=self.race_start_x,
                y=lane_y,
                color=beetle_colors[i],
                name=beetle_names[i],
                base_speed=random_speed,
                lane_y=lane_y
            )
            beetle.odds = random_odds
    ...
    def reset_game(self):
        ...
        for beetle in self.beetles:
            ...
            beetle.is_boosted = False  # Reset boosta
            beetle.boost_timer = 0     # Reset timera boosta
        self.in_game = True
        ...
        for beetle in self.beetles:
            beetle.reset_position()
            beetle.is_boosted = False
            beetle.boost_timer = 0
            # Nowe losowe wartości
            beetle.base_speed = random.uniform(0.7, 1.5)
            beetle.odds = round(random.uniform(1.8, 4.5), 1)
            beetle.wins = random.randint(0, 50)  # Nowe statystyki
            beetle.races = random.randint(beetle.wins, 100)
    ...
    def update(self, delta_time):
        ...
        if self.game_state == "racing":
            if self.pre_race_timer > 0:
                self.pre_race_timer -= delta_time
    ...
    def draw_exit_button(self):
        """Rysuje przycisk wyjścia"""
        # Sprawdź czy mysz jest nad przyciskiem
        mouse_pos = pygame.mouse.get_pos()
        is_hovered = self.exit_button_rect.collidepoint(mouse_pos)
        
        # Kolor przycisku zależny od najechania
        button_color = (80, 40, 40) if is_hovered else (60, 30, 30)
        border_color = (150, 80, 80) if is_hovered else (120, 60, 60)
        
        # Tło przycisku
        pygame.draw.rect(self.screen, button_color, self.exit_button_rect, border_radius=5)
        pygame.draw.rect(self.screen, border_color, self.exit_button_rect, 2, border_radius=5)
        
        # Tekst przycisku
        exit_text = self.font_small.render("WYJŚCIE", True, (255, 255, 255))
